...
func SetDevice(device C.ulong) int {
...
func SetChanAttr(handle uint, attr uint, value float32) uint {
	r := uint(C.BASS_ChannelSetAttribute(C.ulong(handle), C.ulong(attr), C.float(value)))
	return r
}

func GetChanAttr(handle uint, attr uint) float32 {
	var value float32
	C.BASS_ChannelGetAttribute(C.ulong(handle), C.ulong(attr), (*C.float)(unsafe.Pointer(&value)))
	return value
}
...
func StreamCreate(freq uint, proc *C.STREAMPROC, user unsafe.Pointer) uint {
	return uint(C.BASS_StreamCreate(C.ulong(freq), 2, C.BASS_SAMPLE_FLOAT, proc, user))
}
...
func StreamCreateFile(mem int, file string, offset uint64, length uint64) uint {
	return uint(C.BASS_StreamCreateFile(C.int(mem), unsafe.Pointer(C.CString(file)), C.ulonglong(offset), C.ulonglong(length), C.ulong(C.BASS_SAMPLE_FLOAT)))
}
...
func StreamCreateURL(url string, offset uint, proc *C.DOWNLOADPROC, user unsafe.Pointer) uint {
	return uint(C.BASS_StreamCreateURL(C.CString(url), C.ulong(offset), C.BASS_SAMPLE_FLOAT, proc, user))
}

func StreamPutData(handle uint, buffer []byte, length int) uint32 {

	return uint32(C.BASS_StreamPutData(C.ulong(handle), C.CBytes(buffer), C.ulong(length)))
}

func StreamFree(handle uint) uint32 {
	return uint32(C.BASS_StreamFree(C.ulong(handle)))
}
...
func ChannelBytes2Seconds(handle uint, pos int) int {
	return int(C.BASS_ChannelBytes2Seconds(C.ulong(handle), C.ulonglong(pos)))
}
...
func ChannelSeconds2Bytes(handle uint, pos int) int {
	return int(C.BASS_ChannelSeconds2Bytes(C.ulong(handle), C.double(pos)))
}
...
func ChannelIsActive(handle uint) int {
	return int(C.BASS_ChannelIsActive(C.ulong(handle)))
}
...
func ChannelGetPosition(handle uint, mode int) int {
	return int(C.BASS_ChannelGetPosition(C.ulong(handle), C.ulong(mode)))
}
...
func ChannelSetPosition(handle uint, pos int, mode int) int {
	return int(C.BASS_ChannelSetPosition(C.ulong(handle), C.ulonglong(pos), C.ulong(mode)))
}

// ChannelSetAttribute Sets the value of a channel's attribute.
func ChannelSetAttribute(handle uint, attrib C.ulong, value C.float) int {
	return int(C.BASS_ChannelSetAttribute(C.ulong(handle), C.ulong(attrib), value))
}

// ChannelUpdate Updates the playback buffer of a stream or MOD music.
func ChannelUpdate(handle uint, length C.ulong) int {
	return int(C.BASS_ChannelUpdate(C.ulong(handle), C.ulong(length)))
}

// ChannelGetLength Retrieves the playback length of a channel.
func ChannelGetLength(handle uint, mode int) int {
	return int(C.BASS_ChannelGetLength(C.ulong(handle), C.ulong(mode)))
}
...
func PluginLoad(file string) int {
	result := int(C.BASS_PluginLoad(C.CString(file), C.ulong(0)))
	switch result {
...
func PluginFree(handle int) bool {
	result := int(C.BASS_PluginFree(C.ulong(handle)))
	if result == 0 {
		return false
	}
	return true
